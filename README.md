...
1. 修改后面的优化部分，例如part3/4给出更多的提示
2. 可以考虑增加 claude code skill 的优化部分，就是用现在的策略库，汇总成一个文件，让llm直接从文件中挑选一个优化策略用于优化当前函数，然后进一步给出该策略的详细信息，然后让llm给出优化结果
3. 在代码库中加入更多SemOpt的内容，例如把前100个代码库的筛选过程的所有中间结果都复制过来，就可以更直观地看到整个流程和中间结果。至少也应该把策略库放过来？
4. 策略库中没有相对复杂的策略，可能是聚类的问题，也可能是前面筛选流程的问题。
   1. 聚类流程也许可以修改，目前将更常出现的内容放到聚类中，而不常出现的就变成噪点被丢掉。
      1. 出现频率更高的有可能都是相对比较简单的优化策略，所以最后只有简单的策略，没有复杂的策略
      2. 简单的优化策略在不同地方出现的时候，更有可能被总结成基本一致的策略描述；但复杂的策略在不同地方出现的时候，可能被总结成截然不同的文字内容，例如可能细节上有一些区别，最后总结的结果就很不一样，但实际上程序员可能认为他们基本是一类策略。这导致复杂策略就算出现频率比较高也可能无法被聚类。
   2. 筛选流程如果需要修改，可能就是最后用llm筛选的这一步可以修改？
