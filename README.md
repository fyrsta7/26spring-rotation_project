...
聚类结果位于 `commit_list/0_76.json`，其中 `0_76` 表示使用的相似度阈值为 0.76。
...
所有代码库的优化 commits 汇总数据位于 `commit_list/all_is_opt_final.json`，该文件包含了所有代码库的优化 commits 元数据（2529 个 commits）。你可以使用根目录下的 `aggregate_commits.py` 脚本重新生成该汇总文件。
...
测试数据集：
在这一部分中，使用来自 [RocksDB](https://github.com/facebook/rocksdb) 的 commits 作为测试数据。每个 commit 只修改了一个函数。具体内容在 `part2/rocksdb_benchmark.json` 中。
...
测试数据集位于 `part2/rocksdb_benchmark.json`，包含来自 [RocksDB](https://github.com/facebook/rocksdb) 代码库的多个 commits。这些 commits 都满足以下条件：
...
虽然这些 commit 都是只修改了一个函数的小规模修改，但部分修改内容依然可能非常复杂，对于一个并不了解该代码库的程序员来说，也很难快速读懂每一处修改的目的。建议你选择 5-10 个 commits，手动分析它们的优化策略。你可以结合 commit message 和代码修改内容进行分析，也可以让大模型辅助分析。
...
可能的技术方案：
1. 单轮对话：直接在 prompt 中给出需要被优化的代码，并要求大模型分析代码中可能存在的代码优化问题并给出优化结果。

2. 使用 CoT：引导大模型先阅读并理解给出的代码，然后分析有什么代码优化的可能（大模型可能会给出多个方向，也可以在 prompt 中加入限制，要求大模型给出不超过三个的优化方向），然后依次要求大模型给出每种方向上的优化结果。
...
#### 3.3.2 - Syntax Correctness

代码是否可以编译通过。可以使用编译器检查语法错误。

#### 3.3.3 - Semantic Equivalence
...
关于测试：
- RocksDB commits 可以参考 `part2/rocksdb_test.md` 来尝试运行 unit test 和 performance test（如果运行不起来的话手动判断就可以了）
...
方法 2：基于优化类型
```python
def retrieve_by_optimization_type(target_code, commit_list, top_k=3):
    """
...
流程：
1. 构建策略库：从 `commit_list/0_76.json` 中提取优化策略
